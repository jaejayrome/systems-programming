...
/* 4: Char State (Inside a char literal) */
/* 6: Multi-Line Comment End State */
...
int stateMachine(char prevCharacter, char incomingCharacter, int incomingState)
{
  ...
  case '/':
    ...
    if (incomingState == 6 && prevCharacter == '*')
...
int main(void)
{
  ...
  result[0] = '\0';
  int originalCommentNumber = 0; /* Used to determine the starting comment line number*/
  ...
  while (1)
  {
    ...
    if (temp == 1 && currentState == 7)
    {
      ...
      originalCommentNumber = lineNumber;
